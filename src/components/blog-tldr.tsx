 ...
 * paragraph. This block is the single most-read element on the page after
 * the title, and it doubles as the "summary" Google may surface in AI
        ...
        <ul className="space-y-2 text-[15px] leading-relaxed text-[var(--ink-2)]">
          {lines.map((l, i) => (
            <li key={i} className="flex gap-3">
              <span aria-hidden className="mt-2 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--m-accent)]" />
              <span>{l.replace(/^\s*[-*]\s+/, "")}</span>
            </li>
          ))}
        </ul>
      ) : (
        <p className="text-[15px] leading-relaxed text-[var(--ink-2)]">{trimmed}</p>
      )}
    </aside>
  );
}
