  ...
  const addUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data?.error || (locale === "ru" ? "Не удалось добавить" : "Failed to add user"));
      }
    } finally {
      setBusy(false);
    }
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--ink-4)]">
          {locale === "ru"
            ? "Администраторы и менеджеры этого инстанса. Уборщики добавляются через карточки уборщиков на странице объекта."
            : "Admins and managers of this instance. Cleaners are added per-property via the cleaner cards."}
        </p>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-[var(--line)] bg-[var(--bg-2)]">
        {!loaded ? (
          ...
          <div className="p-5 text-sm text-[var(--ink-4)]">
            {locale === "ru" ? "Пользователей пока нет." : "No users yet."}
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--line)] text-left text-[11px] font-medium uppercase tracking-wide text-[var(--ink-4)]">
                ...
                <th className="px-4 py-2.5">{t("settings.created")}</th>
              ...
              {users.map((u, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-[var(--ink-3)]">
                    {new Date(u.createdAt).toLocaleDateString(locale === "ru" ? "ru-RU" : "en-GB", {
                      day: "2-digit",
                      month: "short",
                      year: "numeric",
                    })}
                    ...
                    <td className="px-2 py-3 text-right">
                      {u.role !== "superadmin" && (
                        <button
                          ...
                          aria-label={locale === "ru" ? "Удалить пользователя" : "Delete user"}
                        >
